  ...
  return (
    ...
    >
      ...
      <Link href={link} target={"_blank"} className="inline-block rounded-lg overflow-hidden w-full">
        <FramerImage src={img} alt={title} className="w-full max-h-80 object-cover" whileHover={{ scale: 1.05 }} transition={{ duration: 0.2 }} sizes="100vw" priority />
      </Link>
...
export default function Articles() {
  return (
    <>
      <Head>
        <title>Development Articles | By ShreesatPortfolio</title>
      ...
      <main className={`w-full mb-16 flex flex-col items-center justify-center dark:text-light overflow-hidden`}>
        <Layout className="pt-16">
          <AnimatedText text="Words Influence the World 🌎" className="!text-8xl !leading-tight mb-16 lg:!text-7xl sm:!text-6xl xs:!text-4xl sm:mb-8" />
          <ul className="grid grid-cols-2 gap-16 lg:gap-8 md:grid-cols-1 md:gap-y-16">
            <FeaturedArticle img={blog1} title="Unlocking the Power of Gatsby & Netlify" time="2 min read" summary="Image-centric Gatsby theme for publishers, portfolio, photographers blogs and more." link="https://travislord.xyz/articles/guide-to-clay-theme-gatsby-web-app" />
